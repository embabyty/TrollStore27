...
struct Device {
    ...
    let isOnCoreTrustSupportedVersion: Bool
    ...
    init() {
        ...
        // Check if the device is on a firmware that is only supported through the
        // CoreTrust (CMS SignerInfo) install method, where no kernel exploit is required
        let buildPrefix3 = String(buildNumberStr.prefix(3))
        if buildNumberStr.hasPrefix("23G82") // iOS 26.6.1
        || (buildPrefix3 >= "24A" && buildPrefix3 <= "24Z") // iOS 27.0 RC - 27.x
        {
            self.isOnCoreTrustSupportedVersion = true
        } else {
            self.isOnCoreTrustSupportedVersion = false
        }
        ...
        if self.cpuFamily == .A8 {
            ...
            isSupported = (self.version <= Version("16.6.1")) || (self.isOnSupported17Beta && !((self.cpuFamily == .A15 && !isM2) || self.cpuFamily == .A16)) || self.isOnCoreTrustSupportedVersion
        }
    }
    
    var supportsCoreTrustInstall: Bool {
        return self.isOnCoreTrustSupportedVersion
    }
