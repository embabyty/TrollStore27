...
// Spawn a binary as root (uid 0) using the POSIX persona API.
// Requires com.apple.private.persona-mgmt entitlement, which the CoreTrust
// bypassed build of this app carries. Used by the CoreTrust install method,
// where no kernel exploit is available to escalate privileges.
int run_binary_root(NSString* path, NSArray* args, NSString** output)
{
    NSMutableArray* argsM = args.mutableCopy;
    [argsM insertObject:path.lastPathComponent atIndex:0];
