...
// CoreTrust install method (iOS 26.6.1 (23G82) / iOS 27.0 RC - 27.x (24A-24Z))
// This app is signed with the CoreTrust bypass (CVE-2023-41991) and the required
// private entitlements, so we can spawn the bundled root helper as root (persona)
// without any kernel exploit. Scripts the same flow as TrollHelper:
//   1. install TrollStore from TrollStore.tar (install-trollstore)
//   2. register this app as the user persistence helper (register-user-persistence-helper)
func doCoreTrustInstall(_ device: Device) async -> Bool {
    Logger.log("Running on an \(device.modelIdentifier) on iOS \(device.version.readableString)")
    Logger.log("Using CoreTrust install method (no kernel exploit required)")
    
    guard let bundledHelper = Bundle.main.path(forResource: "trollstorehelper", ofType: "") else {
        Logger.log("trollstorehelper not found in app bundle", type: .error)
        return false
    }
    
    // Prefer an updated TrollStore.tar downloaded to Documents, fall back to the bundled one
    let docsTar = docsDir + "/TrollStore.tar"
    let bundledTar = Bundle.main.path(forResource: "TrollStore", ofType: "tar")
    guard let tarPath = fileManager.fileExists(atPath: docsTar) ? docsTar : bundledTar else {
        Logger.log("TrollStore.tar not found", type: .error)
        return false
    }
    
    Logger.log("Installing TrollStore")
    var installOutput: NSString?
    let installRet = run_binary_root(bundledHelper, ["install-trollstore", tarPath], &installOutput)
    if let output = installOutput {
        print("trollstorehelper output: \(output)")
    }
    if installRet != 0 {
        Logger.log("Failed to install TrollStore (trollstorehelper returned \(installRet))", type: .error)
        return false
    }
    Logger.log("Successfully installed TrollStore!", type: .success)
    
    // Register this app as the user persistence helper so app registrations can be refreshed
    // from this app if they ever revert to "User" state
    if let bundleID = Bundle.main.bundleIdentifier {
        Logger.log("Registering \(bundleID) as user persistence helper")
        let helperRet = run_binary_root(bundledHelper, ["register-user-persistence-helper", bundleID], nil)
        if helperRet == 0 {
            Logger.log("Successfully registered as persistence helper!", type: .success)
        } else {
            Logger.log("Failed to register as persistence helper (\(helperRet))", type: .error)
        }
    }
...
// Refresh TrollStore app registrations (like TrollHelper / TrollStore settings)
func refreshAppRegistrationsCoreTrust() {
    guard let bundledHelper = Bundle.main.path(forResource: "trollstorehelper", ofType: "") else {
        Logger.log("trollstorehelper not found in app bundle", type: .error)
        return
    }
    
    Logger.log("Refreshing app registrations")
    let ret = run_binary_root(bundledHelper, ["refresh"], nil)
    if ret == 0 {
        Logger.log("Successfully refreshed app registrations, respringing", type: .success)
        // Same as TrollHelper: refresh registrations then respring
        DispatchQueue.global().async {
            sleep(2)
            restartBackboard()
        }
    } else {
        Logger.log("Failed to refresh app registrations (\(ret))", type: .error)
    }
}
