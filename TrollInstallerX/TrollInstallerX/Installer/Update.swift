...
// https://api.github.com/repos/embabyty/TrollStore27/releases/latest
// https://github.com/embabyty/TrollStore27/releases/latest/download/TrollStore.tar
...
let bundledVersion = Version("2.1.1")
func getUpdatedTrollStore() async {
    var outOfDate = false
    var doneChecking = false
    var newVersion = ""
    let documentsURL = FileManager.default.urls(for: .documentDirectory, in: .userDomainMask)[0]
    github_fetchLatestVersion("embabyty/TrollStore27", { version in
    ...
    if outOfDate {
        do {
            let newFile = try await downloadFile(from: URL(string: "https://github.com/embabyty/TrollStore27/releases/latest/download/TrollStore.tar")!)
